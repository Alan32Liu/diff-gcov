...
#Thuanpv - declare a dictionary keeping all lines of code that have been changed
#key is base name of the source file, value is a list keeping all updated code in the source file
changes={}
DiffMode = False
...
class CoverageData(object):
    ...
    def __init__(
        ...
        # Thuanpv - initilize list of lines of code that have been changed
        self.basename = os.path.basename(fname).strip()
        self.changes = []
        if self.basename in changes:
            self.changes = changes[self.basename]
            print str(len(self.changes)) + " Lines of CODE & NON-CODE CHANGES in " + self.basename + ":" + str(self.changes)                       
    ...
    # Thuanpv - finalize the data for covered and uncovered lines of code
    # Take into account the code changes
    def finalize(self):
        if len (self.changes) == 0:
            self.changes = list(self.all_lines)

        self.all_lines.intersection_update(self.changes)
        self.uncovered.intersection_update(self.changes)
        for k in self.covered.keys():
    ...
    def summary(self):
        ...
        # (Thuanpv - comment) For line coverage
...
def print_text_report(covdata):
    ...
    if not options.show_covered_branch:
      # Header
      OUTPUT.write("-" * 78 + '\n')
      ...
      for key in keys:
          if DiffMode:
              # Thuanpv - only update if the source file has changed
...
def print_html_report(covdata, details):
    ...
    for key in covdata.keys(): 
        if DiffMode:
            # Thuanpv - only update if the source files has changed       
            basename = os.path.basename(key)
            if basename not in changes:
                continue
        (total, covered, percent) = covdata[key].coverage()
...
parser.add_option(
    "-D", "--diff",
    help="Full path to diff file annotated with line numbers",
    action="store",
    dest="diff",
    default=None
)
...
parser.add_option(
    "-c", "--covered",
    help="Show covered branches, not missing branches",
    action="store_true",
    dest="show_covered_branch",
    default=None
)
...
#
# Thuanpv - Get list of code changes
#
...
if options.diff is not None:
    ...
    sourceFileName = ""
    fd = open(options.diff,'r')
    for line in fd:        
        if line.startswith("diff --git"):
            #sample: diff --git a/png.c b/png.c
            #extract source file
            tokens = line.split(' ')
            if len(tokens) == 4:
                sourceFileName = os.path.basename(tokens[3].strip())
                changes[sourceFileName] = []
                continue
        tempStrs = pattern.findall(line)
        if len(tempStrs) > 0:
            #sample: 2184:+   a = v1 >> 16;
            #extract line number
            tempStrs2 = line.split(':')
            if len(tempStrs2) > 0:
                lineNum = int(tempStrs2[0].strip())
                if sourceFileName in changes:
                    changes[sourceFileName].append(lineNum)
    fd.close()
...
#
# Thuanpv - Update CovData to filter out unwanted part
#

if DiffMode:
    if options.diff is not None:
        for key in covdata:
            covdata[key].finalize()
